...
The app below places 6 points exactly on a line, then fits polynomials of degree 1 through 5 to them. Drag the noise slider to add random noise to every point at once — at 0 the points are perfectly colinear, and as the slider increases they drift farther and farther from the line — and watch what happens to each fit. Click "Regenerate points" to draw a brand-new random line and a fresh set of offsets, or use the checkboxes to show or hide individual fits.
...
The degree-5 polynomial has 6 free coefficients — exactly as many as there are data points. It can therefore pass through all 6 points exactly, no matter how much noise you add: its training error stays pinned near zero the entire time. If you judged these three models purely by how well they fit the data you gave them, the degree-5 fit would always look best.

But watch the curve itself, not just the error number. As you raise the noise slider, the degree-5 curve doesn't just track the underlying line — it swings by large amounts across the whole domain, oscillating to pass through every point's noisy position exactly. Try panning or zooming out on the plot, too: a high-degree fit is sampled well beyond the visible window, so scrolling out reveals just how fast it grows or decays once you're past the data it was trained on. The degree-1 and degree-2 fits barely react at all: they don't have enough parameters to chase the noise in any single point, so they stay close to the overall trend. Toggle the degree-3 and degree-4 checkboxes to see the middle ground: each extra parameter buys a little more flexibility, and a little more sensitivity to the exact data.
