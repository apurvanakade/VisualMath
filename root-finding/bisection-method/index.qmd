...
```{ojs}
//| output: false
// VM is the shared utility library, loaded globally via _includes/head-scripts.html (js/**). This page uses VM.linearRegression, VM.makeFunction, VM.makeNumber, VM.paddedRange, VM.renderTable.
VM = window.VM
```
...
convergencePlot = {
  ...
  const fit = VM.linearRegression(regressionPoints)
  if (fit) {
