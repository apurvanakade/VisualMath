...
//   tolerance    — stop when |f(m)| or interval width is smaller than this
...
precompute = ({fText, a0, b0, tolerance, maxIterations}) => {
  ...
  while (n < maxIterations) {
    ...
    if (Math.abs(fm) < tol || Math.abs(curB - curA) < tol) return {rows, f}
...
```{ojs}
//| output: false
result = precompute({fText, a0, b0, tolerance, maxIterations})
```
