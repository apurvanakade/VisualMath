...
iteratesPlot = {
  const mAccessor = d => {
    if (Number.isFinite(d.m)) return d.m
    return null
  }
  return Plot.plot({
    ...
    y: {label: "mₙ"},
    marks: [
      Plot.lineY(result.rows, {x: "n", y: mAccessor, stroke: "#2563eb"}),
      Plot.dot(result.rows, {x: "n", y: mAccessor, fill: "#2563eb", r: 4})
    ]
  })
}
```
...
convergencePlot = {
  ...
  for (const r of result.rows) {
    if (r.n > 0 && Number.isFinite(r.fm) && r.fm !== 0) {
      logRows.push({n: Math.log10(r.n), y: Math.log10(Math.abs(r.fm))})
    }
  }
