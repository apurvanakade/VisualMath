...
iteratesPlot = Plot.plot({
  ...
  y: {label: "mₙ"},
  marks: [
    Plot.lineY(result.rows, {x: "n", y: d => Number.isFinite(d.m) ? d.m : null, stroke: "#2563eb"}),
    Plot.dot(result.rows, {x: "n", y: d => Number.isFinite(d.m) ? d.m : null, fill: "#2563eb", r: 4})
  ]
})
```
...
convergencePlot = {
  const logRows = result.rows
    .filter(r => r.n > 0 && Number.isFinite(r.fm) && r.fm !== 0)
    .map(r => ({n: Math.log10(r.n), y: Math.log10(Math.abs(r.fm))}))
