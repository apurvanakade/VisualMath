...
result = {
  ...
  const start0 = Number(committed.x0)
  const start1 = Number(committed.x1)

  if (!Number.isFinite(start0) || !Number.isFinite(start1)) return {rows: [], points: [], f}

  const points = [{x: start0, fx: f(start0)}, {x: start1, fx: f(start1)}]
  const rows = []
...
iteratesPlot = {
  ...
  for (let k = 0; k < result.points.length; k++) {
    ...
    if (Number.isFinite(p.x)) ptRows.push({i: k, x: p.x})
  }
  return Plot.plot({
    ...
    y: {label: "xₙ"},
    marks: [
      Plot.lineY(ptRows, {x: "i", y: "x", stroke: "#2563eb"}),
      Plot.dot(ptRows, {x: "i", y: "x", fill: "#2563eb", r: 4})
    ]
  })
}
```
