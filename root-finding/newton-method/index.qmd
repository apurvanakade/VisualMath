...
```{ojs}
//| output: false
// VM is the shared utility library, loaded globally via _includes/head-scripts.html (js/**). This page uses VM.linearRegression, VM.makeDerivative, VM.makeFunction, VM.makeNumber, VM.paddedRange, VM.renderTable.
VM = window.VM
```
...
maxIterations = {
  ...
  const parsed = VM.makeNumber(math, maxIterationsInput)
...
committed = {
  ...
  const x0Val = VM.makeNumber(math, x0Text)
...
convergencePlot = {
  ...
  const fit = VM.linearRegression(logRows)
  if (fit) {
