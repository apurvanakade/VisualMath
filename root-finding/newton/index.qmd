...
viewof fText = {
  ...
  const el = Inputs.text({label: "Function f(x)", value: params.get("f") ?? "x^2", placeholder: "Example: x^3 - x - 2, cos(x) - x"})
  el.style.width = "100%"
  return el
}
```
...
viewof plotTrigger = {
  const el = Inputs.button("Plot", {value: 0, reduce: v => v + 1})
  // Inputs' form wrapper defaults to width: 100% (capped at 360px), which
  // is much wider than the button itself and forces this row to wrap.
  el.style.width = "auto"
  el.style.flex = "none"
  return el
}
...
viewof maxIterations = {
  const el = Inputs.number({value: 20, min: 1, step: 1, label: "Max"})
  // Inputs' form wrapper defaults to width: 100% (capped at 360px), which
  // is much wider than the number field itself and forces this row to wrap.
  el.style.width = "auto"
  el.style.flex = "none"
  return el
}
...
result = {
  ...
  const rows  = []
  const start = Number(committed.x0)
...
```{ojs}
// convergencePlot draws log₁₀|f(xₙ)| vs n (semi-log) with a linear
// regression line, for the entire run — it doesn't depend on the current
// step, so it's just plotted once per run.
convergencePlot = {
  const logRows = []
  for (const r of result.rows) {
    if (Number.isFinite(r.fx) && r.fx !== 0) {
      logRows.push({n: r.i, y: Math.log10(Math.abs(r.fx))})
    }
  }
