...
setup = {
  ...
  const N = 700
  const sampleXs = Array.from({length: N}, (_, i) => xBuffer.lo + (xBuffer.hi - xBuffer.lo) * i / (N - 1))
  ...
  const yValues = [0, ...sampleXs.map((x, i) => x >= xRange.lo && x <= xRange.hi ? sampleYs[i] : null).filter(y => y !== null)]
