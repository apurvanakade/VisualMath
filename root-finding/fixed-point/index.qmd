...
setup = {
  ...
  return {visibleRows, range, bufferRange, xs, ys, x0: Number(committed.x0)}
}
```
...
viewof gText = {
  ...
  const el = Inputs.text({label: "Function g(x)", value: params.get("g") ?? "cos(x)", placeholder: "Example: cos(x), sqrt(x + 2), exp(-x)"})
  el.style.width = "100%"
  return el
}
```
