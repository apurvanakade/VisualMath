...
//   x0           — initial guess
//   tolerance    — stop when |g(x) - x| is smaller than this
...
precompute = ({gText, x0, tolerance, maxIterations}) => {
  ...
  while (i < maxIterations) {
    ...
    // Stop when x and g(x) are close enough — we've found a fixed point.
    if (error < tol) return {rows, g}
...
```{ojs}
//| output: false
result = precompute({gText, x0, tolerance, maxIterations})
```
