...
```{ojs}
//| output: false
// VM is the shared utility library loaded from js/make-function.js, js/make-derivative.js, js/make-number.js, js/make-rational.js, js/padded-range.js, js/render-table.js, js/cubic-spline.js, js/make-ode-function.js.
VM = window.VM
```
...
stages = {
  ...
  const parsed = VM.makeNumber(math, stagesInput)
  let n = 2
...
viewof tableauInput = {
  ...
  const makeCoeffInput = (initialValue, onChange) => {
    ...
    input.addEventListener("input", () => {
      const parsed = VM.makeRational(input.value)
  ...
  const stageRowLatex = i => {
    ...
    const symbolNames = []
    for (let j = 0; j < s; j++) {
      const parsed = VM.makeRational(state.a[i][j])
  ...
  const bRowLatex = () => {
    ...
    const symbolNames = []
    for (let j = 0; j < s; j++) {
      const parsed = VM.makeRational(state.b[j])
      if (parsed === null) coeffs.push(rat.of(0))
      else coeffs.push(parsed)
      symbolNames.push(`k_{${j + 1}}`)
    }
...
tableau = {
  ...
  for (let i = 0; i < s; i++) {
    const row = []
    for (let j = 0; j < s; j++) {
      const parsed = VM.makeRational(tableauInput.a[i][j])
  ...
  const b = []
  for (let j = 0; j < s; j++) {
    const parsed = VM.makeRational(tableauInput.b[j])
