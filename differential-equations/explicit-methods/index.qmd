...
result = {
  ...
  while (n <= maxN) {
    ...
    const rk4Value = rk4Solve(f, t0, y0, b, n).ys[n]
...
setup = {
  ...
  const rk4 = rk4Solve(result.f, t0, y0, b, n)
...
convergencePlot = {
  ...
  const improvedEulerFit = regression(improvedEulerPts)
  if (improvedEulerFit) {
  ...
  const rk4Fit = regression(rk4Pts)
  if (rk4Fit) {
