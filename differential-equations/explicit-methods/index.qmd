...
result = {
  ...
  while (n <= maxN) {
    ...
    const rk4Value = VM.rk4Solve(f, t0, y0, b, n).ys[n]
...
setup = {
  ...
  const rk4 = VM.rk4Solve(result.f, t0, y0, b, n)
...
convergencePlot = {
  ...
  const improvedEulerFit = VM.linearRegression(improvedEulerPts)
  if (improvedEulerFit) {
  ...
  const rk4Fit = VM.linearRegression(rk4Pts)
  if (rk4Fit) {
