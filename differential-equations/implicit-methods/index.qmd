...
Because $y_{i+1}$ appears on both sides, each step requires solving a root-finding problem $g(y_{i+1}) = y_{i+1} - y_i - h\, f(t_{i+1}, y_{i+1}) = 0$. This page solves it with a few steps of [Newton's method](../../root-finding/newton-method/index.qmd), starting from the explicit Euler step as the initial guess and using a numerical estimate of $\partial f/\partial y$ in place of $g'$.
...
```{ojs}
viewof maxN = {
  const maxNInput = Inputs.number({value: 20, min: 1, step: 1, label: "Max"})
  maxNInput.classList.add("ojs-auto")
  return maxNInput
}
```
...
```{ojs}
//| output: false
// eulerSolve(f, t0, y0, tEnd, n) advances y' = f(t, y), y(t0) = y0 with n
// steps of Euler's method and returns the full trajectory {ts, ys}.
eulerSolve = (f, t0, y0, tEnd, n) => {
  const h = (tEnd - t0) / n
  const ts = [t0]
  const ys = [y0]
  let t = t0, y = y0
...
```{ojs}
//| output: false
// rk4Solve(f, t0, y0, tEnd, n) advances y' = f(t, y), y(t0) = y0 with n
// steps of the classical fourth-order Runge-Kutta method. Returns the full
// trajectory {ts, ys}. Used only as the high-accuracy reference below —
// RK4 is not one of the checkboxes on this page.
rk4Solve = (f, t0, y0, tEnd, n) => {
  const h = (tEnd - t0) / n
  const ts = [t0]
  const ys = [y0]
  let t = t0, y = y0
...
result = {
  ...
  // tested below.
  const refSteps = 2000
  const trueValue = rk4Solve(f, t0, y0, b, refSteps).ys[refSteps]

  const rows = []
  let n = 1
  while (n <= maxN) {
    const eulerValue = eulerSolve(f, t0, y0, b, n).ys[n]
...
convergencePlot = {
  ...
  const eulerFit = regression(eulerPts)
  if (eulerFit) {
  ...
  const implicitEulerFit = regression(implicitEulerPts)
  if (implicitEulerFit) {
