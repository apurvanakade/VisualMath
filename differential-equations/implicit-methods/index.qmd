...
```{ojs}
//| output: false
// VM is the shared utility library, loaded globally via _includes/head-scripts.html (js/**). This page uses VM.eulerSolve, VM.linearRegression, VM.makeFunction2, VM.makeNumber, VM.paddedRange, VM.renderTable, VM.rk4Solve.
VM = window.VM
```
...
result = {
  ...
  let n = 1
  while (n <= maxN) {
    const eulerValue = VM.eulerSolve(f, t0, y0, b, n).ys[n]
...
setup = {
  ...
  const euler = VM.eulerSolve(result.f, t0, y0, b, n)
...
convergencePlot = {
  ...
  const eulerFit = VM.linearRegression(eulerPts)
  if (eulerFit) {
  ...
  const implicitEulerFit = VM.linearRegression(implicitEulerPts)
  if (implicitEulerFit) {
