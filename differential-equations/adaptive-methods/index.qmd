...
```{ojs}
//| output: false
// VM is the shared utility library loaded from js/make-function.js, js/make-derivative.js, js/padded-range.js, js/render-table.js, js/cubic-spline.js, js/make-ode-function.js.
VM = window.VM
```

The classical RK4 method in [Explicit Methods](../explicit-methods/index.qmd) advances with a fixed step size $h = b/n$ everywhere — spending the same effort even where the solution is nearly linear. **RKF45** (Runge–Kutta–Fehlberg) instead adapts $h$ at every step, driven by a target tolerance $\varepsilon$ rather than a fixed step count.
...
\begin{aligned}
k_1 &= f(t_i, y_i), \\
k_2 &= f\!\left(t_i + \tfrac{1}{4}h,\ y_i + \tfrac{1}{4}h k_1\right), \\
k_3 &= f\!\left(t_i + \tfrac{3}{8}h,\ y_i + h\left(\tfrac{3}{32}k_1 + \tfrac{9}{32}k_2\right)\right), \\
k_4 &= f\!\left(t_i + \tfrac{12}{13}h,\ y_i + h\left(\tfrac{1932}{2197}k_1 - \tfrac{7200}{2197}k_2 + \tfrac{7296}{2197}k_3\right)\right), \\
k_5 &= f\!\left(t_i + h,\ y_i + h\left(\tfrac{439}{216}k_1 - 8k_2 + \tfrac{3680}{513}k_3 - \tfrac{845}{4104}k_4\right)\right), \\
k_6 &= f\!\left(t_i + \tfrac{1}{2}h,\ y_i + h\left(-\tfrac{8}{27}k_1 + 2k_2 - \tfrac{3544}{2565}k_3 + \tfrac{1859}{4104}k_4 - \tfrac{11}{40}k_5\right)\right),
\end{aligned}
$$
...
viewof y0 = {
  const params = new URLSearchParams(window.location.search)
  const raw = params.get("y0")
  let value = 0.5
  if (raw !== null && Number.isFinite(Number(raw))) value = Number(raw)
  return Inputs.number({label: "Initial condition y(0)", value})
}
```
...
```{ojs}
viewof maxSteps = {
  const maxStepsInput = Inputs.number({value: 2000, min: 1, step: 1, label: "Max steps"})
  maxStepsInput.classList.add("ojs-auto")
  return maxStepsInput
}
```
...
committed = {
  ...
  const y0Val = Number((viewof y0).value)
  const bVal = Number((viewof bEnd).value)

  const params = new URLSearchParams(window.location.search)
  params.set("f", fVal)
  params.set("y0", String(y0Val))
...
```{ojs}
//| output: false
// rk4Reference(f, t0, y0, tEnd, n) advances y' = f(t, y), y(t0) = y0 with n
// fixed steps of the classical fourth-order Runge-Kutta method — used only
// to build a fine reference solution to compare the adaptive run against.
rk4Reference = (f, t0, y0, tEnd, n) => {
  const h = (tEnd - t0) / n
  const ts = [t0]
  const ys = [y0]
  let t = t0, y = y0
...
result = {
  ...
  const f = VM.makeFunction2(math, committed.fText)
  ...
  const t0 = 0
  const y0 = Number(committed.y0)
  const b = Number(committed.bEnd)
  ...
  const maxStepsClamped = Math.max(1, Math.round(Number(maxSteps)))
  const solved = rk45Solve(f, t0, y0, b, tolerance, maxStepsClamped)
  ...
  const refSteps = 2000
  const trueValue = rk4Reference(f, t0, y0, b, refSteps).ys[refSteps]
...
setup = {
  ...
  const reference = rk4Reference(result.f, t0, y0, b, 500)

  const tRange = VM.paddedRange([t0, b], {emptyRange: [-3, 3], relativePadding: 0.1, minPadding: 0.25})
