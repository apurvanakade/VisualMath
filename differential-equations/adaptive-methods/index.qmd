...
```{ojs}
//| output: false
// VM is the shared utility library, loaded globally via _includes/head-scripts.html (js/**). This page uses VM.makeFunction2, VM.makeNumber, VM.paddedRange, VM.renderTable, VM.rk4Solve.
VM = window.VM
```
...
result = {
  ...
  const refSteps = 2000
  const trueValue = VM.rk4Solve(f, t0, y0, b, refSteps).ys[refSteps]
...
setup = {
  ...
  const reference = VM.rk4Solve(result.f, t0, y0, b, 500)
