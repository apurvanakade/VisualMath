...
```{ojs}
//| output: false
// VM is the shared utility library, loaded globally via _includes/head-scripts.html (js/**). This page uses VM.lagrangeQuadratic, VM.linearRegression, VM.makeFunction, VM.makeNumber, VM.paddedRange, VM.renderTable, VM.simpsonEstimate.
VM = window.VM
```
...
result = {
  ...
  const trueIntegral = VM.simpsonEstimate(f, lo, hi, 4000)
  ...
  while (n <= maxN) {
    ...
    let simpsonArea = NaN
    if (n % 2 === 0) simpsonArea = VM.simpsonEstimate(f, lo, hi, n)
...
setup = {
  ...
  if (n % 2 === 0) {
    ...
    for (let i = 0; i < n; i += 2) {
      const panel = VM.lagrangeQuadratic(nodesX[i], nodesY[i], nodesX[i + 1], nodesY[i + 1], nodesX[i + 2], nodesY[i + 2], samplesPerPair)
...
convergencePlot = {
  ...
  const leftFit = VM.linearRegression(leftPts)
  if (leftFit) {
  ...
  const midpointFit = VM.linearRegression(midpointPts)
  if (midpointFit) {
  ...
  const trapezoidalFit = VM.linearRegression(trapezoidalPts)
  if (trapezoidalFit) {
  ...
  const simpsonFit = VM.linearRegression(simpsonPts)
  if (simpsonFit) {
