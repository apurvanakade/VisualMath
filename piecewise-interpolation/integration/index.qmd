...
// Simpson's rule using n subintervals. n must be even.
...
result = {
  ...
  while (n <= maxN) {
    ...
    let simpsonArea = NaN
    if (n % 2 === 0) simpsonArea = simpsonEstimate(f, lo, hi, n)
...
setup = {
  ...
  if (n % 2 === 0) {
    ...
    for (let i = 0; i < n; i += 2) {
      const x0 = nodesX[i],     y0 = nodesY[i]
      const x1 = nodesX[i + 1], y1 = nodesY[i + 1]
      const x2 = nodesX[i + 2], y2 = nodesY[i + 2]
      for (let k = 0; k <= samplesPerPair; k++) {
        const x = x0 + (x2 - x0) * k / samplesPerPair
        const l0 = ((x - x1) * (x - x2)) / ((x0 - x1) * (x0 - x2))
        const l1 = ((x - x0) * (x - x2)) / ((x1 - x0) * (x1 - x2))
        const l2 = ((x - x0) * (x - x1)) / ((x2 - x0) * (x2 - x1))
        simpsonX.push(x)
        simpsonY.push(y0 * l0 + y1 * l1 + y2 * l2)
      }
    }
  }
...
convergencePlot = {
  ...
  const leftFit = regression(leftPts)
  if (leftFit) {
  ...
  const midpointFit = regression(midpointPts)
  if (midpointFit) {
  ...
  const trapezoidalFit = regression(trapezoidalPts)
  if (trapezoidalFit) {
  ...
  const simpsonFit = regression(simpsonPts)
  if (simpsonFit) {
