...
```{ojs}
//| output: false
// VM is the shared utility library loaded from js/make-function.js, js/make-derivative.js, js/padded-range.js, js/render-table.js, js/cubic-spline.js.
VM = window.VM
```

A fixed grid of $n$ equally spaced subintervals, as in [Numerical Integration](../integration/index.qmd), spends the same effort everywhere — even on parts of $[a,b]$ where $f$ is nearly linear and a coarse panel would already be accurate. **Adaptive quadrature** instead refines the partition only where it is needed, driven by a target tolerance $\varepsilon$ rather than a fixed panel count.
...
```{ojs}
viewof maxDepth = {
  const maxDepthInput = Inputs.number({value: 15, min: 1, step: 1, label: "Max depth"})
  maxDepthInput.classList.add("ojs-auto")
  return maxDepthInput
}
```
...
```{ojs}
//| output: false
// simpsonEstimate(f, lo, hi, n) approximates ∫_lo^hi f(x) dx with composite
// Simpson's rule using n subintervals (n must be even) — used only to build
// a fine reference integral to compare the adaptive estimate against.
simpsonEstimate = (f, lo, hi, n) => {
  const h = (hi - lo) / n
  let sum = 0
  const y0 = f(lo)
  const yn = f(hi)
...
result = {
  ...
  const a = Number(committed.a0)
  const b = Number(committed.b0)
  ...
  const depthLimit = Math.max(1, Math.round(Number(maxDepth)))
  ...
  const trueIntegral = simpsonEstimate(f, lo, hi, 4000)
...
setup = {
  ...
  const panels = []
  for (const leaf of result.leaves) {
    const x0 = leaf.a,   y0 = leaf.fa
    const x1 = leaf.mid, y1 = leaf.fmid
    const x2 = leaf.b,   y2 = leaf.fb
    const xs = [], ys = []
    for (let k = 0; k <= samplesPerLeaf; k++) {
      const x = x0 + (x2 - x0) * k / samplesPerLeaf
    ...
    panels.push({xs, ys, depth: leaf.depth})
  }
  ...
  const xRange = VM.paddedRange([lo, hi], {emptyRange: [-3, 3], relativePadding: 0.1, minPadding: 0.25})
  ...
  const xBuffer = VM.paddedRange([lo, hi], {emptyRange: [-6, 6], relativePadding: 0.25, minPadding: 0.5})
...
decisionTreePlot = {
  ...
  const colorRange = ["#94a3b8", "#16a34a"]
