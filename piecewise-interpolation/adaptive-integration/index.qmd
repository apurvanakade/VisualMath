...
```{ojs}
//| output: false
// VM is the shared utility library, loaded globally via _includes/head-scripts.html (js/**). This page uses VM.lagrangeQuadratic, VM.makeFunction, VM.makeNumber, VM.paddedRange, VM.renderTable, VM.simpsonEstimate.
VM = window.VM
```
...
setup = {
  ...
  const panels = []
  for (const leaf of result.leaves) {
    const sample = VM.lagrangeQuadratic(leaf.a, leaf.fa, leaf.mid, leaf.fmid, leaf.b, leaf.fb, samplesPerLeaf)
