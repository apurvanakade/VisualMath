...
```{ojs}
// Referenced (but otherwise unused) inside teaserColorMap below so that
// clicking the button re-runs the random color assignment without moving
// either slider.
viewof teaserRegenerate = {
...
```{ojs}
teaserMorpher = createTriangleMorpher(teaserN, vertices, VM.discreteMath.spernerColor)

teaserColorMap = {
  teaserRegenerate;
  return teaserMorpher.buildColorMap();
}

teaserMorphed = teaserMorpher.interpolatePoints(teaserT, teaserColorMap)
teaserTriangles = teaserMorpher.generateSubTriangles(teaserMorphed.morphedMap)
teaserPlot = plotTriangles(teaserTriangles, teaserMorphed, teaserN)
```
...
Sperner’s condition is crucial in asserting that $|\Delta_t|$ is constant. If the condition is violated, $\Delta_t$ is no longer a triangulation of the original triangle — even for small $t > 0$ — and thus we cannot conclude that $|\Delta_t|$ remains constant. You can see this in the app below: the left panel keeps Sperner's condition, the right panel deliberately breaks it.
